...
export default function Imagepassword() {
  ...
  const [array, setArray] = useState([...Array(32)].map((_, index) => index));
  ...
  useEffect(() => {
    ...
    const initCheck =async ()=>{
      try{
        const docRef = doc(db, "celeb_graphical_password_4x8_final",localStorage.getItem("name"));
...
if(true){
  ...
  //  }
   ...
   if(docSnap.data().setup.toString()===selectedNumbers.toString()){
    await setDoc(doc(db, "celeb_graphical_password_4x8_final",localStorage.getItem("name"),"attempts",`recall-${Date.now()}`), {
    ...
    await setDoc(doc(db, "celeb_graphical_password_4x8_final",localStorage.getItem("name")), {
      timestamp: new Date().toString(),
